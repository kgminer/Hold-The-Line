...
Implementation of user story 10.
A paddle in the game has a collider attached to it. When a ball enters this collider, it triggers the OnTriggerEnter() method in the paddle class. The method adds the ball to a list that the paddle keeps track of to determine if there is a ball in the area for it to grab. Then in the Update() method, it checks to see if there are balls to grab and that the player hit the button to activate the grab. If this is the case then the paddle stops the ball in place and moves it along with the paddle while the player is holding down the button. Once the button is release the ball is given a velocity that sends it away from the ball. Upon leaving the collider, the method OnTriggerExit() is called which removes the ball from the list so it is no longer detected by the paddle.
...
Help:
The Help menu deals with user story 3 in that it displays the information to help the player learn the game. Once again, interactions are the same as the previous menus, using the joystick to move around to the button of choice and then hitting A to select that button. It also deals with user stories 25  and 30. The camera and audio settings are both able to changed using the Settings section of the Help menu.
